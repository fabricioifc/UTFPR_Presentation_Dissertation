...
A perda de entropia cruzada é uma medida da diferença entre a distribuição de probabilidade prevista (\(p_i\)) e o \textit{ground truth} dos rótulos para a classe (\(y_i\)), comumente expressa pela Equação \ref{eq:celoss} \cite{Celoss2019}.
...
\noindent
onde \(L_{CE}\) é a perda de entropia cruzada, \(y\) é o \textit{ground truth}, \(p\) é o vetor de probabilidades atribuídas a cada classe. A soma é realizada sobre todas as classes.
