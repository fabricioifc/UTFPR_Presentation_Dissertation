...
\begin{table}[!ht]
    ...
    \mode<presentation>{\scriptsize}%% Modo apresentação: tamanho de fonte
    \mode<article>{\small}%% Modo artigo: tamanho de fonte
    \caption{Comparação dos resultados obtidos.}%% Legenda
    \label{tab:res:comparacao}%% Rótulo
...
% Crie um frame com uma tabela comparativa entre os quatro cenários de experimentos. A tabela deve conter os seguintes dados: acurácia, precisão, sensibilidade, F1-Score e IoU. A tabela deve ser criada com base nos r
% esultados obtidos

\begin{frame}
\frametitle{Comparação dos Resultados}
\framesubtitle{Acurácia}

\begin{figure}[!htb]
\centering%
\caption{Comparação da acurácia obtida nos cenários de experimentos.}%
\label{fig:res:acuracia}
\mode<presentation>{\includegraphics[width = 0.8\columnwidth]{./Figuras/boxplot/boxplot_acuracia-min.png}}%% Modo apresentação: tamanho da figura
